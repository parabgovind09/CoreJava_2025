...
//        These are objects whose state cannot be changed once they are created. 
//        Strings in Java are immutable. The method concat() does not modify the original string but instead creates a new string and returns it. 
//        Since the reference in the for-each loop is declared as final, you cannot reassign the String reference, but you can still invoke methods like concat() on the string. 
//        However, since strings are immutable, the original string will remain unchanged, and the concatenation will not be saved.
        
        for (final String name : names1) {
            System.out.println(name);
            System.out.println(name.concat(" Lokesh")); // Modifying the object's internal state is allowed it will create a new object which will be discarded as it is not stored and strings are immutable in java 
...
//	It can be initialized in the instance block or constructor only. The blank final variable can be static also which will be initialized in the static block only.
...
final class ImmutablePerson {
    ...
    public ImmutablePerson(String name, int age, Address address) {
        ...
        System.out.println(System.identityHashCode(this.address));
    }
